...
public class printUtil {
    ...
    public static void printInfoTable(String infoName,String[][] info, int start, String... colName){
        StringBuilder s = new StringBuilder();
        StringBuilder sLine = new StringBuilder();
        int[] tableSize = new int[colName.length];
        for (int i = 0; i < tableSize.length; i++) {
            tableSize[i] = configUtil.getConfigInt(colName[i]); // id/name level hp mana weapon armor
        }
        s.append("|%") ;
        sLine.append("+");
        for (int j : tableSize) {
...
//        Name/cost/required level/attribute increase/attribute affected
        System.out.printf(s.toString(),colName);
        System.out.println(sLine);
        for (int i = 0; i < info.length; i++) {
            String[] target = info[i];
            System.out.printf(s.toString(),target[0], target[2],
...
//        Name/cost/required level/attribute increase/attribute affected
    ...
    public static void printObjectInfoTable(String infoName, List<? extends Tablefiable> info, int start, String... colName){
        String[][] sinfo = new String[info.size()][];
        for (int i = 0; i < sinfo.length; i++) {
            sinfo[i] = info.get(i).getAttrString();
        }
        printInfoTable(infoName, sinfo, start, colName);
    }
